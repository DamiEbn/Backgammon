...
int main(){
    ...
    //einlesen durch funktionsaufruf readFile
    int players_count = readPlayers(players, MAX_PLAYERS);
    if (players_count != 2) {
    ...
    while (players[0].rolled_out != 15
        ...
        int moves = 0;
        //prüfen ob push
        if (players[which].dice[0] == players[which].dice[1]){
            moves = 4;
        }
        else moves = 2;
        //drucken des index
        ...
        while (moves_counter != moves) {
            ...
            if (moves == 2) {
                ...
                //umwandlung des strings in eine zahl
                char *endptr;
                int used_num = strtol(digit_number_move, &endptr, 10);
                //wenn es sich bei der Eingabe um keine Zahl handelt
                if (endptr == digit_number_move) {
                    fprintf(stderr, "Eingabefehler:\n");
                    exit(5);
                }
                //prüfen ob es sich um eine zahl im gültigkeitsberich handelt
